...
parser <- arg_parser("Pseudobulk Wilcoxon")
...
if (length(cond_levels) != 2) {
	stop(sprintf("wilcox requires exactly 2 conditions, found: %d", length(cond_levels)))
}
...
safe_wilcox <- function(x1, x2) {
	if (length(x1) < 1 || length(x2) < 1) {
		return(NA_real_)
	}
	if (all(x1 == x1[1]) && all(x2 == x2[1]) && x1[1] == x2[1]) {
		return(1.0)
	}
	suppressWarnings(tryCatch(wilcox.test(x2, x1, exact = FALSE)$p.value, error = function(e) NA_real_))
}

p_values <- vapply(
	seq_len(nrow(sample_mean_cpm)),
	function(i) safe_wilcox(sample_mean_cpm[i, group1], sample_mean_cpm[i, group2]),
	numeric(1)
)
